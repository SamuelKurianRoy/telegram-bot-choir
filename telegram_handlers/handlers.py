        ...
        try:
            if len(parts) == 3:
                try:
                    ...
                    input_date = date(year, month, min(day, last_day))
            elif len(parts) == 2:
                ...
                input_date = date(current_year, current_month, min(day, last_day))
            else:
                return "Invalid date format. Use DD, DD/MM, DD/MM/YY, or DD/MM/YYYY."
        except Exception as e:
            return f"Invalid date format. Use DD, DD/MM, DD/MM/YY, or DD/MM/YYYY."
...
async def admin_check_ai_model(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if ai_assistant._sarvam_client is not None:
            ...
            response_lines.append(f"   Model: `sarvam-m`")
        else:
    ...
    try:
        ...
        if ai_assistant._sarvam_client is None:
            ...
            try:
                ...
                response_text = test_response.choices[0].message.content.strip()
                results.append(f"✅ Working!")
                results.append(f"   Model: `sarvam-m`")
                results.append(f"   Response: {response_text[:30]}")
            except Exception as e:
                error_str = str(e)
